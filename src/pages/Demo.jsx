...
// Must match your deployed Modal fastapi_app URL.
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
        <span className="text-[13px] font-medium text-gray-900">Analysis Result</span>
        <span
      ...
      <div className="p-4 space-y-4 text-[13px]">
        ...
        {result.user_story && (
          <div>
            <h4 className="font-semibold text-emerald-800 uppercase tracking-wider text-[10px] mb-1">
              Refined User Story
            </h4>
        ...
        {result.acceptance_criteria && result.acceptance_criteria.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-700 uppercase tracking-wider text-[10px] mb-1">
              Acceptance Criteria
            </h4>
            <ul className="list-disc pl-5 space-y-1 text-gray-700 bg-gray-50 border border-gray-100 rounded-lg p-3">
        ...
        {result.raw_output && (
          <div>
            <h4 className="font-semibold text-gray-400 uppercase tracking-wider text-[10px] mb-1">
            ...
            if (msg.role === "user") {
              return (
                <div key={i} className="flex justify-end">
                  <div className="bg-indigo-600 text-white text-[13px] rounded-2xl rounded-br-sm px-4 py-2.5 max-w-md">
                    {msg.text}
                  </div>
                </div>
              );
            ...
            if (msg.role === "error") {
              return (
                <div key={i} className="flex justify-start">
                  <div className="bg-red-50 border border-red-200 text-red-700 text-[13px] rounded-2xl rounded-bl-sm px-4 py-2.5 max-w-md">
                    {msg.text}
                  </div>
                </div>
              );
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-end gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 focus-within:border-indigo-300 transition">
            <textarea
              ...
              rows={1}
              disabled={isLoading}
              className="flex-1 bg-transparent resize-none text-[13px] text-gray-900 placeholder-gray-400 focus:outline-none py-1.5 max-h-32"
            />
            <button
              onClick={handleSend}
              disabled={isLoading || !inputText.trim()}
              className="shrink-0 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-[12px] font-medium rounded-lg transition"
            >
